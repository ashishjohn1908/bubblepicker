...
data class Item(val pickerItem: PickerItem, val circleBody: CircleBody) {
    ...
    private fun createBitmap(isSelected: Boolean): Bitmap {
        ...
        if (pickerItem.withImage) {
            if (isSelected) drawImage(canvas)
            drawBackground(canvas, isSelected)
        } else {
            if (pickerItem.withImage) drawImage(canvas)
            drawBackgroundSelected(canvas, isSelected)
        }
        drawIcon(canvas)
        drawText(canvas)
    ...
    private fun drawBackgroundSelected(canvas: Canvas, isSelected: Boolean) {
        val bgPaint = Paint()
        bgPaint.style = Paint.Style.FILL
        pickerItem.color?.let { bgPaint.color = pickerItem.color!! }
        pickerItem.colorSelected?.let { bgPaint.color = pickerItem.colorSelected!! }
        if (isSelected) {
            bgPaint.color = pickerItem.colorSelected!!
        } else {
            bgPaint.color = pickerItem.color!!
        }
        canvas.drawRect(0f, 0f, bitmapSize, bitmapSize, bgPaint)
